...
class KTModel(nn.Module):
  def __init__(self, n_skill, n_diff, n_hints, n_time_used, max_len, emb_dim, bs):
    super().__init__()
  ...
  def forward(self, skill, answer, diff, hints, time_used):
    ...
    y_preds = []
    y_preds.append(torch.zeros(self.bs).to(self.device))
    ...
    for t in range(self.max_len-1):
      ...
      score_1 = torch.sigmoid(q_1 * k_1 / torch.sqrt(torch.ones(self.bs, self.emb_dim).to(self.device) * self.emb_dim))
      out_1 = v_1 * score_1

      q_2 = self.W_q_2 * x_t
      k_2 = self.W_k_2 * c_t
      v_2 = self.W_v_2 * hints_t
      score_2 = torch.sigmoid(q_2 * k_2 / torch.sqrt(torch.ones(self.bs, self.emb_dim).to(self.device) * self.emb_dim))
      out_2 = v_2 * score_2
      ...
      k_3 = self.W_q_3 * h_t
      v_3 = self.W_v_3 * a_t
      score_3 = torch.sigmoid(q_3 * k_3 / torch.sqrt(torch.ones(self.bs, self.emb_dim).to(self.device) * self.emb_dim))
      out_3 = v_3 * score_3
